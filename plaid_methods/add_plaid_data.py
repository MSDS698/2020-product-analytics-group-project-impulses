...
def add_accounts(accounts_data, user_id):
    ...
    with db.connect() as conn:
        select_statement = select([meta.tables['dw.plaid_items'].c.plaid_id]).where(
            meta.tables['dw.plaid_items'].c.user_id == user_id)
        result_set = conn.execute(select_statement)
        for r in result_set:
            plaid_id = r[0]
        for account in accounts:
            insert_statement = meta.tables['dw.accounts'].insert().values(user_id=user_id, plaid_id=plaid_id,
    ...
    with db.connect() as conn:
        for transaction in transactions:
            select_statement = select(
                [meta.tables['dw.accounts'].c.user_id, meta.tables['dw.accounts'].c.account_id]).where(
                meta.tables['dw.accounts'].c.account_plaid_id == transaction['account_id'])
            result_set = conn.execute(select_statement)
            for r in result_set:
                user_id = r[0]
                account_id = r[1]
            ...
            insert_statement = meta.tables['dw.transaction'].insert().values(user_id=user_id,
