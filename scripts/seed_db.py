...
def create_dummy_user(user,sandbox_options=None, habits=None):
    ...
    user = classes.User(user.fname, user.lname,user.email,user.phone,user.password)
    db.session.add(user)
    public_token = sandbox.PublicToken(client).create('ins_109508',initial_products=['transactions'], _options=sandbox_options)['public_token']
    ...
    checking_account = [account for account in accounts if account['name'] == "Checking"][0]
    add_plaid_data.add_accounts([checking_account],user,user.plaid_items[0], commit=False)
    transactions = methods.get_transactions(client,'2019-10-01','2019-11-01',plaid.access_token,checking_account['account_id'])
